...
```{r histogram01, fig.width=6, fig.height=4}
library(plyr)
total_steps <- ddply(mydata  
                     ,.(date)  
                     ,summarise  
                     ,nas=sum(is.na(steps))  
                     ,N=length(interval)  
...
par(mar=c(5,6,2,1), mgp=c(3,.75,0), las=1)
...
```{r summary, echo=TRUE}
list(mean=mean(total_steps$steps, na.rm=TRUE),
...
```{r time series plot, fig.width=6, fig.height=4}
tseries <- ddply(mydata
                 ,.(interval)
                 ,summarise
...
maximum number of steps is `r paste(max_steps_hour, max_steps_mins, sep=":")` am.
...
mydata$stepsWinput <- ifelse(is.na(mydata$steps)
                             ,impNA
                             ,mydata$steps)
...
```{r hitogram2, fig.width=6, fig.height=4}
total_steps2 <- ddply(mydata2
                     ,.(date)
                     ,summarise
                     ,steps=sum(stepsWinput,na.rm=TRUE)
                     )
par(mar=c(5,6,2,1), mgp=c(3,.75,0), las=1)
...
list(mean=mean(total_steps2$steps, na.rm=TRUE),
     median=median(total_steps2$steps, na.rm=TRUE))
```
...
```{r time series plot 2, fig.width=6, fig.height=4}
tseries2 <- aggregate(stepsWinput ~ interval + tday
                     ,mydata2
                     ,mean
                     ,na.rm=TRUE)
par(mar=c(5,6,2,1), mgp=c(3,.75,0), las=1)
library(ggplot2)
ggplot(tseries2, aes(x=interval, y=stepsWinput)) +
  facet_grid(tday ~.) +
  geom_line() +
  xlab("Interval") +
